...
@Injectable()
export class ReactivateOrganizationUnitTypeUseCase {
  constructor(
  ...
  execute = async (
    ...
    const type = await this.organizationUnitsRepository.setTypeStatus(tenantId, typeId, "ACTIVE");

    await this.createAuditEventUseCase.execute({
      tenantId,
      actorUserId,
      action: "organization_unit_type.reactivated",
