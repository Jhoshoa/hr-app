...
describe("OrganizationUnit use cases", () => {
  ...
  it("creates tenant-scoped organization unit types", async () => {
    ...
    const useCase = new CreateOrganizationUnitTypeUseCase(repository, policy);
  ...
  it("rejects duplicate organization unit type keys", async () => {
    ...
    await expect(
      useCase.execute({ tenantId: "tenant-1", key: "branch", name: "Branch" })
  ...
  it("blocks archiving organization unit types used by active units", async () => {
    ...
    const useCase = new ArchiveOrganizationUnitTypeUseCase(repository, policy);

    await expect(useCase.execute("tenant-1", "type-1")).rejects.toBeInstanceOf(ConflictException);
  ...
  it("creates organization units with active type, parent, and primary location", async () => {
    ...
    const useCase = new CreateOrganizationUnitUseCase(repository, policy);
    const result = await useCase.execute({
      tenantId: "tenant-1",
      typeId: "type-1",
  ...
  it("rejects hierarchy cycles when updating organization units", async () => {
    ...
    const useCase = new UpdateOrganizationUnitUseCase(repository, policy);
  ...
  it("blocks archiving organization units with active children", async () => {
    ...
    const useCase = new ArchiveOrganizationUnitUseCase(repository, policy);

    await expect(useCase.execute("tenant-1", "unit-1")).rejects.toBeInstanceOf(ConflictException);
